...
describe('data plane repository', () => {
  ...
  it('rotates shared manifest signing key when active private_key_ref cannot be resolved locally', async () => {
    ...
    const getActiveManifestSigningKeyRecord = vi.fn(() =>
      Promise.resolve(
        Array.from(records.values()).find(record => record.status === 'active') ?? null
      )
    )
    ...
    const rotatedKey = await repository.getManifestSigningPrivateKeyShared()
    expect(rotatedKey.kid).not.toBe(localKey.value.private_key.kid)
    expect(rotatedKey.kid).not.toBe(remoteActiveKey.value.private_key.kid)
    ...
    expect(mutableRepository.state.manifest_signing_active_private_key_ref).toBe(
      `state://manifest-signing-key/${rotatedKey.kid}`
    )
    const rotatedKeySet = repository.getManifestVerificationKeys()
    expect(rotatedKeySet.keys.some(key => key.kid === rotatedKey.kid)).toBe(true)
    expect(rotatedKeySet.keys.some(key => key.kid === remoteActiveKey.value.private_key.kid)).toBe(true)
    ...
    expect(retireManifestSigningKey.mock.invocationCallOrder[0]).toBeLessThan(
      setActiveManifestSigningKey.mock.invocationCallOrder[0]
    )
  ...
  it('uses shared db repositories for session, policy, approval, and manifest rule paths', async () => {
    ...
    const dbRepositories = {
      ...
      templateRepository: {
        ...
        getLatestTemplateByTenantTemplateId: vi.fn(
      ...
      sessionRepository: {
        ...
        getSessionByTokenHash: vi.fn(() => Promise.resolve({
          sessionId: 'sess_1',
          workloadId: 'w_1',
          tenantId: 't_1',
          certFingerprint256: 'AA:BB:CC',
      ...
      approvalRequestRepository: {
        ...
        createApprovalRequestFromCanonicalDescriptor: vi.fn(() => Promise.resolve({
          approval_id: 'apr_1',
          status: 'pending',
    ...
    ;(dbRepositories.sessionRepository.getSessionByTokenHash as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      null
    )
  ...
  it('fails bootstrap when tolerated rotation conflicts do not converge to local signing key', async () => {
    ...
    const secretRepository = {
      ...
      setActiveManifestSigningKey: vi.fn(() =>
        Promise.reject(
          Object.assign(new Error('invalid activation transition'), {code: 'state_transition_invalid'})
        )
      ),
      listManifestVerificationKeysWithEtag: vi.fn(() =>
        Promise.resolve({
    ...
    const internals = repository as unknown as {
      cryptoStorageService: {
        createManifestSigningKeyRecord_INCOMPLETE: (
          input: {kid: string; alg: 'EdDSA' | 'ES256'; public_jwk: unknown; private_key_ref: string; created_at: string}
  ...
  it('uses redis-backed crypto rotation lock adapter when redis is available', async () => {
    ...
    const storage = (repository as unknown as {
      cryptoStorageService: {
        acquireCryptoRotationLock_INCOMPLETE: (input: {
          lock_name: string
          ttl_ms: number
  ...
  it('throws when shared manifest keyset lookup returns a hard failure', async () => {
    ...
    const secretRepository = (
      (repository as unknown as {processInfrastructure?: ProcessInfrastructure}).processInfrastructure?.dbRepositories as
